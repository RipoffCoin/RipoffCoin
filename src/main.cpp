...
// Copyright (c) 2013-2100 RipoffCoin developers
...
uint256 hashGenesisBlock("0xacff2573a15b87576be4c65ea80f3f1196423eea0c4c65d6f85ee2a6804f54a8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // RipoffCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "RipoffCoin Signed Message:\n";
...
{
    // RipoffCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 50 * COIN; // 50 RIPO for each block, should be worth 1 BTC soon
    ...
    // Subsidy is cut in half every 100000 blocks, which will occur ... open calculator yourself. 2 minutes * 100,000
    nSubsidy >>= (nHeight / 100000); // RipoffCoin: 100k blocks

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // RipoffCoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // RipoffCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // RipoffCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // RipoffCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // RipoffCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x52f026941a873437696adeabdb099cca0a408852c94f0df540431ab8591fda50");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "well, that escalated quickly";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1404242420; //epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1505482;
        if (fTestNet)
        {
            block.nTime    = 1404242421;
            block.nNonce   = 1038016;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xa17a6add37085813fc18570b9b0eb3f24b9bbfbcb6065e4825c600ded754b062"));
////
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // RipoffCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RipoffCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RipoffCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RipoffCoinMiner(CWallet *pwallet)
{
    printf("RipoffCoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running RipoffCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RipoffCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RipoffCoinMiner, pwallet));
}
